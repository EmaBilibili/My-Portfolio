...
const ProjectCard = ({ project, accentColor, hoverColor }) => (
    ...
    >
        <div className="relative h-48 overflow-hidden border-b border-white/5">
            ...
            >
                {project.language === 'en' && project.category_en ? project.category_en : project.category}
            </div>
        </div>

        <div className="p-6">
            <h3 className="text-xl font-bold text-white mb-2 font-heading group-hover:transition-colors" style={{ '--hover': accentColor }}>
                {project.language === 'en' && project.title_en ? project.title_en : project.title}
            </h3>
            <p className="text-gray-400 text-sm mb-6 line-clamp-2 leading-relaxed">{project.language === 'en' && project.description_en ? project.description_en : project.description}</p>
            ...
            <div className="flex items-center gap-4">
                ...
                >
                    <Play className="w-4 h-4" /> <span className="uppercase tracking-wide">{project.t('projects.demo')}</span>
                </a>
                {project.repoLink && project.repoLink !== "#" && (
                    ...
                    >
                        <Github className="w-4 h-4" /> <span className="uppercase tracking-wide">{project.t('projects.code')}</span>
                    </a>
                )}
            </div>
        </div>
                    ...
                    <div className="flex flex-col gap-10">
                        ...
                        <div className="space-y-10">
                            {webProjects.map((project, index) => (
                                <ProjectCard key={index} project={{ ...project, language, t }} accentColor="#34d399" hoverColor="secondary" />
                            ))}
                        </div>
                    ...
                    <div className="flex flex-col gap-10">
                        <div>
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-surface border border-primary/20 text-primary text-sm mb-4 font-mono">
                                ...
                                <span>{t('projects.gameTitle')}</span>
                            </div>
                            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold font-heading text-white leading-relaxed break-words">
                                {t('projects.gameHeading')}
                            </h2>
                        </div>

                        <div className="space-y-10">
                            {gameProjects.map((project, index) => (
                                <ProjectCard key={index} project={{ ...project, language, t }} accentColor="#10b981" hoverColor="primary" />
                            ))}
                        </div>
                    </div>
                </div>
